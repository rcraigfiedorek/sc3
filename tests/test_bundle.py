...
class BundleTestCase(unittest.TestCase):
    ...
    @unittest.skipIf(
        ...
        "OSX's UDP packet size is known to be shorter",
    )
    ...
    def test_nested_bundle_time(self):
        ...
        n = NetAddr("127.0.0.1", NetAddr.lang_port())
        self.assertRaises(
            ValueError, lambda n: n.send_bundle(1, ["/msg", 1], [0.5, ["/msg", 2]]), n
        )
        self.assertRaises(
            ValueError, lambda n: n.send_bundle(0, ["/msg", 1], [None, ["/msg", 2]]), n
        )
    ...
    @unittest.skipIf(
        sys.platform.startswith("win32"), "Windows fails the test, I can't check why"
    )
    ...
    def test_bndl_processing_time(self):
        ...
        def func(msg, *_):
            ...
            for tm in OscPacket(msg[1]).messages:
                result.append(math.modf(SystemClock.osc_to_elapsed_time(tm.time))[0])
            main.resume()
