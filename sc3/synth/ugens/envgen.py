...
class EnvGen(ugn.UGen):
    ...
    def ar(
        ...
        return cls._multi_new(
            "audio", gate, level_scale, level_bias, time_scale, done_action, env
        )
    ...
    def kr(
        ...
        if isinstance(env, evp.Env):
            env = utl.unbubble(env._envgen_format())  # Was asMultichannelArray.
        return cls._multi_new(
            "control", gate, level_scale, level_bias, time_scale, done_action, env
        )
...
class Linen(ugn.UGen):
    ...
    @classmethod
    def kr(
        cls, gate=1.0, attack_time=0.01, sus_level=1.0, release_time=1.0, done_action=0
    ):
        return cls._multi_new(
            "control", gate, attack_time, sus_level, release_time, done_action
        )
