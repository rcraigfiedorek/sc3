...
class TDuty(Duty):
    ...
    def ar(cls, dur=1.0, reset=0.0, level=1.0, done_action=0, gap_first=0):
        return cls._multi_new("audio", dur, reset, done_action, level, gap_first)

    @classmethod
    def kr(cls, dur=1.0, reset=0.0, level=1.0, done_action=0, gap_first=0):
        return cls._multi_new("control", dur, reset, done_action, level, gap_first)
...
class DUGen(ugn.UGen):
    ...
    def linlin(self, inmin, inmax, outmin, outmax, clip="minmax"):
        return (self.prune(inmin, inmax, clip) - inmin) / (inmax - inmin) * (
            outmax - outmin
        ) + outmin

    def linexp(self, inmin, inmax, outmin, outmax, clip="minmax"):
        return (
            bi.pow(outmax / outmin, (self - inmin) / (inmax - inmin)) * outmin
...
class Dpoll(DUGen):
    ...
    def _new1(cls, rate, input, label, run, trig_id):  # override
        ...
        label = [
            int(x) for x in bytes(label, "utf-8")
        ]  # *** TODO: sc ascii method, sclang uses signed values, see Poll.
...
class Dunique(ugn.PseudoUGen):
    ...
    def _init(self, source, max_buffer_size, protected):
        ...
        if self._protected:
            ...
            self._writer = Dbufwr.new(
                source,
                self._buffer,
                Dbufwr.new(Dseries.new(0, 1, 16777216), self._iwr),
                1,
            )
        else:
            self._iwr = None
            ...
            self._writer = Dbufwr.new(
                source,
                self._buffer,
                Dseq.new([Dseries.new(0, 1, max_buffer_size)], float("inf")),
                0,
            )
    ...
    def _as_ugen_input(self, *_):
        ...
        if self._protected:
            ...
            overrun = (Dbufrd.new(self._iwr) - Dbufrd.new(ird)) > self._buffer.frames
            # // Catch buffer overrun by switching to a zero length series.
            brd = Dswitch1.new([brd, Dseries.new(length=0)], overrun)
        else:
            index = Dseq.new([Dseries.new(0, 1, self._buffer.frames)], float("inf"))
