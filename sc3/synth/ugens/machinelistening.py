...
class RunningSum(ugn.UGen):
    ...
    def rms(cls, input, num_samples=40):
        sig = RunningSum.ar(
            input.squared(), num_samples
        )  # NOTE: si se pasa un valor erróneo a input va a tirar un error distinto al habitual.
...
class SpecPcile(ugn.UGen):
    ...
    @classmethod
    def kr(
        cls, chain, fraction=0.5, interpolate=0, binout=0
    ):  # Added binout parameter, supercollider/supercollider#5097 v3.13.0
...
class Onsets(ugn.UGen):
    _default_rate = "control"
    _ODF_TYPES = ("power", "magsum", "complex", "rcomplex", "phase", "wphase", "mkl")
...
class BeatTrack2(ugn.MultiOutUGen):
    ...
    @classmethod
    def kr(
        cls, busindex, numfeatures, windowsize=2.0, paccuracy=0.02, lock=0, wscheme=None
    ):
        wscheme = -2.1 if wscheme is None else wscheme
        return cls._multi_new(
            "control", busindex, numfeatures, windowsize, paccuracy, lock, wscheme
        )
