...
class Convolution2(ugn.UGen):
    ...
    @classmethod
    def ar(
        cls, input, kernel, trigger=0, frame_size=2048
    ):  # NOTE: wrong kernel size crashes the server.
...
class StereoConvolution2L(ugn.MultiOutUGen):
    ...
    @classmethod
    def ar(cls, input, kernel_L, kernel_R, trigger=0, frame_size=2048, crossfade=1):
        return cls._multi_new(
            "audio", input, kernel_L, kernel_R, trigger, frame_size, int(crossfade)
        )
