...
class FFT(PV_ChainUGen):
    ...
    def kr(cls, buffer, input=0.0, hop=0.5, wintype=0, active=1, winsize=0):
        return cls._multi_new("control", buffer, input, hop, wintype, active, winsize)
...
class PV_RectComb2(PV_ChainUGen):
    ...
    def new(cls, buffer_a, buffer_b, num_teeth=0.0, phase=0.0, width=0.5):
        return cls._multi_new("control", buffer_a, buffer_b, num_teeth, phase, width)
...
class PV_JensenAndersen(PV_ChainUGen):
    ...
    def ar(
        ...
        return cls._multi_new(
            "audio", buf, propsc, prophfe, prophfc, propsf, threshold, wait_time
        )
...
class PackFFT(PV_ChainUGen):
    ...
    @classmethod
    def kr(cls, chain, bufsize, magsphases, frombin=0, tobin=None, zeroothers=0):
