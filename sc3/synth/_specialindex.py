...
_binops_list = [
    ...
    (
        "round",
        "__round__",
    ),  # Is round(x, quant), in Python __round__ is unary with arg.
    ("roundUp", "roundup"),
    (
        "trunc",
        "__trunc__",
    ),  # Is trunc(x, quant), in Python __truc__ is unary, default op for int(x).
    ("atan2",),
    ("hypot",),
    ("hypotApx", "hypotx"),
...
def sc_spindex_opname(operator):
    ...
    """
    try:
        return _unops[operator]
    except KeyError:
        pass
    try:
        return _binops[operator]
    except KeyError:
        pass
    return (-1, None)
