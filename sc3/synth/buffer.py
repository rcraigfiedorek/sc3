...
class Buffer(gpp.UGenParameter, gpp.NodeParameter):
        ...
        *,
        ...
        if alloc:
            if self._frames is None:
                raise ValueError("cannot allocate buffer, frames is None")
    ...
    def new_consecutive(
        ...
        for i in range(buffers):
            ...
            server.addr.send_msg(
                ...
                frames,
                channels,
                fn.value(completion_msg, new_buf, i),
            )
    ...
    @classmethod
    def new_read(
        cls, path, start_frame=0, frames=-1, server=None, bufnum=None, action=None
    ):
        """
        ...
        obj.alloc_read(path, start_frame, frames, lambda buf: ["/b_query", buf.bufnum])
        return obj

    @classmethod
    def new_read_channel(
        cls,
        path,
        start_frame=0,
        frames=-1,
        ...
        obj.alloc_read_channel(
            path, start_frame, frames, channels, lambda buf: ["/b_query", buf.bufnum]
        )
        return obj
    ...
    @classmethod
    def new_load_list(
        cls, lst, channels=1, server=None, action=None
    ):  # Was new_load_collection
        ...
        if server.addr.is_local:
            ...
            path = str(plf.Platform.tmp_dir / ("SC_" + uuid.uuid4().hex + ".wav"))
    ...
    @classmethod
    def new_send_list(
        cls, lst, channels=1, server=None, wait=-1, action=None
    ):  # Was send_collection
        """
        ...
        server = server or srv.Server.default
        lst = list(array.array("f", lst))  # Type check & cast.
        buffer = cls(bi.ceil(len(lst) / channels), channels, server, alloc=False)
    ...
    def alloc_read(self, path, start_frame=0, frames=-1, completion_msg=None):
        """
    ...
    def alloc_read_channel(
        self, path, start_frame=0, frames=-1, channels=None, completion_msg=None
    ):
        """
        Read a sound file on the server for buffer objects previously created
        with ``alloc=False``.
    ...
    def fill(self, start, frames, values):
        ...
        self._server.addr.send_msg("/b_fill", self._bufnum, start, int(frames), *values)
    ...
    def query(self, action=None):
        ...
        rpd.OscFunc(
            resp_func, "/b_info", self._server.addr, arg_template=[self._bufnum]
        ).one_shot()
    ...
    def set(self, index, value, *more_pairs):
        ...
        self._server.addr.send_msg("/b_set", self._bufnum, index, value, *more_pairs)
    ...
    def load_list(self, lst, start_frame=0, action=None):  # Was load_collection
        if self._server.addr.is_local:
            ...
            return self.read(path, buf_start_frame=start_frame, action=remove_file)
        else:
            _logger.warning("cannot call 'load' with a non-local Server")

    def send_list(
        self, lst, start_frame=0, wait=-1, action=None
    ):  # Was send_collection
        lst = list(array.array("f", lst))  # Type check & cast.
        size = len(lst)
        if size > (self._frames - start_frame) * self._channels:
            _logger.warning("lst is larger than available number of frames")
        self._stream_list(lst, size, start_frame * self._channels, float(wait), action)

    def _stream_list(
        self, lst, size, start_frame, wait, action
    ):  # Was streamCollection
        def stream_func():
            ...
            while pos < size:
                ...
                self._server.addr.send_msg(
                    "/b_setn", self._bufnum, start_frame + pos, len(sublst), *sublst
                )
                pos += max_bndl_size
                if wait >= 0:
                    yield wait
    ...
    def get(self, index, action):
        ...
        rpd.OscFunc(
            resp_func, "/b_set", self._server.addr, arg_template=[self._bufnum, index]
        ).one_shot()
    ...
    def getn(self, index, count, action):
        ...
        rpd.OscFunc(
            resp_func, "/b_setn", self._server.addr, arg_template=[self._bufnum, index]
        ).one_shot()
    ...
    def get_to_list(self, action, index=0, count=None, wait=0.01, timeout=3):
        ...
        def resp_func(msg, *_):
            ...
            if msg[1] == self._bufnum:
                ...
                if ref_count <= 0:
                    ...
                    fn.value(
                        action, array
                    )  # , self)  # *** NOTE: CHANGEG: because get & getn don't pass self to action, only data
        ...
        def getn_func():
            ...
            while pos < count:
                ...
                self._server.addr.send_msg("/b_getn", self._bufnum, pos, getsize)
                pos += getsize
                if wait >= 0:
                    yield wait
                else:
    ...
    def _gen_action_responder(self, action):
        if action is not None:
            ...
            rpd.OscFunc(
                ...
                "/done",
                self._server.addr,
                arg_template=["/b_gen", self._bufnum],
            ).one_shot()
    ...
    def sine1(
        self, amps, normalize=True, as_wavetable=True, clear_first=True, action=None
    ):
        if self._bufnum is None:
            raise BufferAlreadyFreed("sine1")
        self._gen_action_responder(action)
        oflags = self._gen_oflags(normalize, as_wavetable, clear_first)
        self._server.addr.send_msg("/b_gen", self._bufnum, "sine1", oflags, *amps)
    ...
    def cheby(
        ...
        self._gen_action_responder(action)
        oflags = self._gen_oflags(normalize, as_wavetable, clear_first)
        self._server.addr.send_msg("/b_gen", self._bufnum, "cheby", oflags, *amps)

    def copy_data(self, dst_buffer, dst_start=0, start=0, num_samples=-1, action=None):
        if self._bufnum is None:
    ...
    def _init_server_cache(cls, server):
        if server not in cls._server_caches:
            ...
            mdl.NotificationCenter.register(
                server, "_new_allocators", cls, lambda: cls._clear_server_caches(server)
            )
